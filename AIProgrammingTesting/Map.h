...
public:
	...
	std::deque<Direction>			GetPathBetweenPoints(int startPointX, int startPointY, int goalPointX, int goalPointY);
	std::pair<int, int>				GetRandomPointInMaze();
	bool							LineOfSight(int originX, int originY, int pointX, int pointY);
	bool							CanMoveInDirectionFromPoint(int originX, int originY, Direction direction, bool adhereToWalls = true);
};